...
/** Остатки: v4/product/info/stocks — filter.visibility обязателен, limit 1–1000. Пагинация по last_id. */
...
/** Все остатки с пагинацией (для раздела Товары — Остатки). */
...
async function getPostingByNumber(postingNumber) {
  ...
  if (result.price_summary != null && Number(result.price_summary) > 0) {
    ...
    products.forEach((p) => {
      const v = Number(p.sum_price ?? p.price ?? p.amount ?? 0);
