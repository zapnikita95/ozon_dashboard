...
/** Список отправлений FBS: POST /v3/posting/fbs/list (filter since/to; pagination). Ozon отдаёт не больше чем за ~месяц — для больших диапазонов используйте getPostingsListRange. */
async function getPostingsList(filter = {}) {
  const since = filter.since || filter.in_process_at_from;
  const to = filter.to || filter.in_process_at_to;
  if (!since || !to) throw new Error('getPostingsList требует filter.since и filter.to (или in_process_at_from / in_process_at_to)');
  const body = { filter: { since, to }, limit: 100, offset: 0 };
  const results = [];
  do {
    const data = await ozonPost('/v3/posting/fbs/list', { filter: body.filter, limit: body.limit, offset: body.offset });
    const list = data.result?.postings ?? (Array.isArray(data.result) ? data.result : []);
    results.push(...list);
...
const POSTINGS_CHUNK_DAYS = 29;

/** Список отправлений за любой период: разбивает диапазон на чанки по 29–30 дней (лимит Ozon ~месяц), склеивает без дубликатов. */
async function getPostingsListRange(sinceIso, toIso) {
  if (!sinceIso || !toIso) throw new Error('getPostingsListRange требует since и to в ISO');
  const start = new Date(sinceIso);
  const end = new Date(toIso);
  if (start >= end) return [];
  const DAY_MS = 24 * 60 * 60 * 1000;
  const chunkMs = POSTINGS_CHUNK_DAYS * DAY_MS;
  const byNum = new Map();
  for (let t = start.getTime(); t < end.getTime(); t += chunkMs) {
    const chunkEnd = Math.min(t + chunkMs, end.getTime());
    const sinceChunk = new Date(t).toISOString().slice(0, 19) + 'Z';
    const toChunk = new Date(chunkEnd).toISOString().slice(0, 19) + 'Z';
    const list = await getPostingsList({ since: sinceChunk, to: toChunk });
    list.forEach((p) => {
      const num = p.posting_number ?? p.id;
      if (num) byNum.set(String(num), p);
    });
  }
